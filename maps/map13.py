...
# load school and classification data
...
def classify_school(logic_class):
    if logic_class.startswith("01") and logic_class.endswith("1"):
        return 'Yellow Triangle'
    elif logic_class.startswith("01") and logic_class.endswith("0"):
        return 'Yellow Circle'
    elif logic_class.startswith("00") and logic_class.endswith("1"):
        return 'Gray Triangle'
    elif logic_class.startswith("00") and logic_class.endswith("0"):
        return 'Gray Circle'
    else:
        return 'Other'

merged["Category"] = merged["LOGIC_CLASS"].apply(classify_school)
...
# load Georgia boundaries, counties, interstates
...
county_zip = tempfile.NamedTemporaryFile(suffix=".zip", delete=False).name
urllib.request.urlretrieve(county_url, county_zip)
counties = gpd.read_file("zip://" + county_zip).to_crs(epsg=3857)
...
road_zip = tempfile.NamedTemporaryFile(suffix=".zip", delete=False).name
...
# legend for boundaries and interstates
...
leg2 = ax.legend(handles=legend_elements_extra,
...
fig.savefig("output/map13_county.png", dpi=300, bbox_inches='tight', bbox_extra_artists=[leg1, leg2])
#plt.show()
